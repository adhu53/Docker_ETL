...
def update_sql_db(eid,ename,esal,ecity,mod):
    while True:
        try:
            conn=pymysql.connect(host="mysql",database="jan",user="root",password="root")
            cursor=conn.cursor()
...
git_url="https://raw.githubusercontent.com/adhu53/Docker_ETL/refs/heads/main/employees_"
today=datetime.datetime.now().strftime("%Y-%m-%d")
git_url=git_url+today+".csv"
print(today)
